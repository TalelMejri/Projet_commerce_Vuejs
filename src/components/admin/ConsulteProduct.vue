<template>
    <div class="container py-5">
                ...
                <div class="col-lg-4 mb-2">
                   <form @submit.prevent="searchData()">
                       <input
                         ...
                         v-model="search"
                       />
                       <button type="submit" class="btn btn-outline-success mt-2">  <i  class="material-icons mx-2 text-dark">search</i></button>
                     </form>
                </div>
       ...
       <div class="row d-flex justify-content-center gap-4 mt-2">
        ...
        <div v-else  v-for="tab in tab_product" :key="tab.id"  class="carde col-lg-4 ">
            ...
            <div class="face face2">
                <div class="content text-dark">
                    <div>
                        ...
                        <p class="text-center"><strong>Quantity</strong> :{{tab.Quantity}}</p>
                        <p class="text-center"><strong>Name Product</strong>{{tab.name_product}}</p>
                        <div class="d-flex gap-4">
                            <button class="btn btn-warning">Edit</button>
                            <button type="button" class="btn btn-danger" data-bs-toggle="modal" :data-bs-target="`#exampleModal${+tab.id}`">delete</button>
                        </div>
                    </div>
                </div>
            </div>

            <div class="modal fade" :id="`#exampleModal${+tab.id}`" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
              <div class="modal-dialog">
               <div class="modal-content">
                  ...
                  <div class="modal-footer">
                    ...
                    <button type="button" @click="deleteProduct(tab.id)" class="btn btn-danger">Delete</button>
                  </div>
                 </div>
                </div>
             </div>
...
export default{
    ...
    methods:{
        ...
        searchData(){
            service.get_all_product(this.search ,this.pagination.current_page)
               .then((response)=>{
        ...
        deleteProduct(id){
            service.delete_product(id).then(()=>{
                service.get_all_product((response)=>{
                    this.tab_product=response.data.data;   
                })
            })
        }
