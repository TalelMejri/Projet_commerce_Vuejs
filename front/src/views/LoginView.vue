...
export default defineComponent({
 ...
 components:{
  ...
  setup() {
    ...
    const login = async () => {
      const response = await authService.login(email.value,pass.value)
      console.log(response);
      if (response){
        console.log(response);
        let isAdmin = JSON.parse(response.isAdmin);
        let isClient = JSON.parse(response.isClient);
        console.log("isAdmin",isAdmin,"isClient",isClient);
        if(isAdmin==true){
        ...
        message_error.value = "an error occured";
      }
    }
